...
If you discover a security issue in `explica` — for example a way to make the generated HTML execute arbitrary code, leak data, contact an external network, or otherwise behave outside its documented surface — please report it privately.
...
- The affected version(s) of `explica` (run `npm view @ojesusmp/explica version` if unsure).
...
`explica` is a documentation-generation skill. Its security surface is intentionally small:
...
- **Out of scope:** Issues with the Claude Code platform itself, the user's terminal, or third-party skills are not handled here — please report those to their respective maintainers.
